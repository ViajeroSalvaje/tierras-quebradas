...
const GRUPOS_HABILIDADES = [
  ...
  { id: "comunicacion", label: "Comunicación (COM)", habs: [
    ...
    { clave: "manipulacion", label: "Manipular" },
    { clave: "oratoria", label: "Oratoria" },
    { clave: "tratarAnimales", label: "Tratar animales" }
  ]},
  { id: "cultura", label: "Cultura (CUL)", habs: [
    { clave: "academia", label: "Academia (E)" },
    { clave: "conocimientoMagico", label: "Conocimiento mágico (E)" },
    { clave: "estrategia", label: "Estrategia (E)" },
    { clave: "idioma2", label: "Idioma 2 (E)" },
    { clave: "idioma3", label: "Idioma 3 (E)" },
    { clave: "leyendas", label: "Leyendas" },
    { clave: "medicina", label: "Medicina (E)" },
    { clave: "memorizar", label: "Memorizar" },
    { clave: "multiverso", label: "Multiverso (E)" },
    { clave: "naturaleza", label: "Naturaleza" },
    { clave: "navegacion", label: "Navegación (E)" },
    { clave: "pociones", label: "Pociones (E)" },
    { clave: "sueños", label: "Sueños (E)" },
  ...
  { id: "tecnica", label: "Técnica (TEC)", habs: [
    { clave: "arco", label: "Arco (E)" },
    { clave: "ballesta", label: "Ballesta" },
    { clave: "canonDeMano", label: "Disparo" },
    { clave: "honda", label: "Honda (E)" },
    { clave: "artesania", label: "Artesanía (E)" },
    { clave: "forzarCerraduras", label: "Forzar cerraduras (E)" },
    { clave: "hurtar", label: "Hurtar" },
    { clave: "manejarCarros", label: "Manejar carros" },
    { clave: "ocultar", label: "Ocultar" },
    { clave: "primerosAuxilios", label: "Primeros auxilios (E)" }
  ]},
  { id: "vigor", label: "Vigor (VIG)", habs: [
    { clave: "armasAsta", label: "Asta" },
    { clave: "escudo", label: "Escudo" },
    { clave: "armasEspada", label: "Espada" },
    { clave: "armasMangos", label: "Mango" },
    { clave: "armasPunhal", label: "Puñal" },
    { clave: "lanzar", label: "Lanzar" },
    { clave: "manejarBotes", label: "Manejar botes (E)" },
...
const IDEOLOGIAS = [
  { id: "ley", label: "La Ley", desc: "Orden, civilización y progreso (Patriarcado / Tres Valles)" },
  { id: "caos", label: "El Caos", desc: "Cambio, libertad y fuerza (Excelsa / Merendrak)" },
  { id: "equilibrio", label: "El Equilibrio Cósmico", desc: "Punto medio entre Orden y Entropía. Sin devoción ni promesas." },
  { id: "elementales", label: "Los Señores Elementales", desc: "Adoración a las fuerzas de la naturaleza" },
  { id: "antepasados", label: "Los Antepasados", desc: "Culto a los ancestros de los Caídos (Imanguk, Jungla)" },
  { id: "sincretismo", label: "Sincretismo", desc: "Adoración conjunta de dioses de la Ley y el Caos (isla de Templanza)" },
  { id: "sinReligion", label: "Sin Religión", desc: "Sin lealtad a ningún panteón. Nunca puede alinearse.", ppBonus: 1 }
];

const DEIDADES_POR_IDEOLOGIA = {
  ley: ["El Guerrero", "El Comerciante", "El Juez", "El Sabio", "El Escriba", "El Sanador", "La Madre", "El Labrador", "El Artista", "La Destructora", "Baaler"],
  caos: ["Aniria", "Ayrok", "Íkor", "Mashak", "Meibel", "Melk", "Nariaj", "Shador", "Shonbark", "Slodar", "Tesh-Chan", "Vesh-Anh", "Yarói", "Baaler"],
  elementales: ["Kamin (Tierra)", "Tepel (Fuego)", "Vodar (Agua)", "Visdu (Aire)"],
  antepasados: [],
  equilibrio: [],
  sincretismo: ["El Guerrero", "El Comerciante", "El Juez", "El Sabio", "El Escriba", "El Sanador", "La Madre", "El Labrador", "El Artista", "La Destructora", "Aniria", "Ayrok", "Íkor", "Mashak", "Meibel", "Melk", "Nariaj", "Shador", "Shonbark", "Slodar", "Tesh-Chan", "Vesh-Anh", "Yarói", "Baaler"],
  sinReligion: []
};

const ACTITUDES = [
  { id: "servicial", label: "Servicial", desc: "Entiende la superioridad absoluta de los dioses y los sirve lealmente" },
  { id: "atemorizado", label: "Atemorizado", desc: "Cumple con la religión por miedo al castigo divino" },
  { id: "pragmatico", label: "Pragmático", desc: "Ve su servicio como un intercambio por poder y beneficios" },
  { id: "rebelde", label: "Rebelde", desc: "Siente rencor hacia los dioses y busca un mundo libre de su influencia" },
  { id: "indiferente", label: "Indiferente", desc: "Intenta permanecer ajeno a las disputas divinas" }
];
...
export class CharacterCreator extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  static ARMA_LABELS = {
    armasEspada: "Espada",
    armasAsta: "Arma de asta",
    armasMangos: "Arma de mango",
    armasPunhal: "Puñal",
    escudo: "Escudo",
    pelea: "Pelea",
    arco: "Arco",
  ...
  _charData = {
    ...
    habilidadesLibres: {},
    // Religión
    ideologiaReligion: null,
    deidad: "",
    actitudDivina: "indiferente",
    plRepartidos: { ley: 0, caos: 0, elementos: 0, antepasados: 0 },
    // Magia
    tipoMagia: null,
    tipoMagiaAuto: false,
    hechizosElegidos: [],
  ...
  async _getHechizos() {
    ...
    this._hechizosCache = docs.map(d => ({
      id:         d.id,
      name:       d.name,
      verbo:      d.system.verbo ?? "",
      esfera:     d.system.esfera ?? "",
      dificultad: d.system.dificultad ?? 0,
      pmCoste:    d.system.pmCoste ?? 1,
      pmMax:      d.system.pmMax ?? 0
    })).sort((a, b) => a.name.localeCompare(b.name));
  ...
  async _prepareContext(options) {
    ...
    if (stepId === "rasgos") {
      ...
      const mapR = r => {
        ...
        return {
          ...r,
          selected:      elegidosMap.has(r.id),
          deshabilitada: !elegidosMap.has(r.id) && r.coste < 0 && ppActual + r.coste < 0,
          costeBadge:    r.coste < 0 ? r.coste + " PP" : "+" + r.coste + " PP",
          esHechizos,
          cantidad:      esHechizos && elegidoEntry ? (elegidoEntry.cantidad ?? 1) : undefined
        };
      };
      rasgosItemsData = {
    ...
    if (stepId === "religion") {
      ...
      const plTotal = {
        ley: pl.ley + bonusAuto.ley,
        caos: pl.caos + bonusAuto.caos,
        elementos: pl.elementos + bonusAuto.elementos,
        antepasados: pl.antepasados + bonusAuto.antepasados
      };
      ...
      const plFilas = ["ley", "caos", "elementos", "antepasados"].map(k => ({
        ...
        total: plTotal[k],
        alineado: alineado[k],
        puedeSubir: plRestante > 0,
        puedeBajar: pl[k] > 0
      }));

      religionData = {
        ideologias: IDEOLOGIAS.map(i => ({ ...i, selected: i.id === ideologia })),
        deidades: deidades.map(d => ({ nombre: d, selected: d === this._charData.deidad })),
        hayDeidades: deidades.length > 0,
        actitudes: actitudesFiltradas.map(a => ({ ...a, selected: a.id === this._charData.actitudDivina })),
        plBase,
        plGastado,
        plRestante,
        plFilas,
        hayBonus: esSacerdote || tieneDevocion || esPuritano,
        esSacerdote,
        tieneDevocion,
        esPuritano,
        lealtadClave,
        ideologiaActiva: ideologia
      };
    }
    ...
    if (stepId === "magia") {
      ...
      const opcTipos = [
        { id: "sin",        label: "Sin magia",       costeStr: "0 PP",                           desc: "Sin capacidad mágica." },
        { id: "don",        label: "Don de la magia", costeStr: "−1 PP",                          desc: "Potencial innato sin entrenamiento. No empieza con hechizos." },
        { id: "hechiceria", label: "Hechicería",      costeStr: this._calcCosteStr("hechiceria"), desc: "Mago entrenado con hechizos, verbos y esferas." },
        { id: "teurgia",    label: "Teúrgia",         costeStr: this._calcCosteStr("teurgia"),    desc: "Magia blanca ligada a la Ley. Solo hechizos de Esfera Ley (máx. 3 ó 4)." }
      ...
      if (tieneHechizos) {
        ...
        listaHechizosAgrupada = esfFiltradas.map(esfera => ({
          label: esfera,
          hechizos: todos.filter(h => h.esfera === esfera).map(h => {
            const sel = this._charData.hechizosElegidos.find(x => x.id === h.id);
            return {
              ...h,
              selected:  !!sel,
              permanent: sel?.permanent ?? false,
              bloqueado: h.dificultad >= 25 || (!sel && atMax),
              pmStr:     h.pmMax ? `${h.pmCoste}–${h.pmMax}` : `${h.pmCoste}`
            };
          })
        })).filter(g => g.hechizos.length > 0);
      }

      const mapSkill = (key, label) => {
        const libre = this._charData.magiaLibres[key] ?? 0;
        const total = baseHechiceria + libre;
        return {
          key, label, base: baseHechiceria, libre, total,
          puedeSubir: total < 8 && puntosRestantes > 0,
          puedeBajar: libre > 0
        };
      };
      ...
      magiaData = {
        ...
        rasgoMagia,
        accesoForzado,
        profesionNombre: this._charData.profesionNombre,
        tipoActual,
        opcTipos,
        tieneHechizos,
        esTeurgia,
        baseHechizos,
        elegidosCount:  this._charData.hechizosElegidos.length,
        extrasCount,
        extrasCost:     extrasCount,
        listaHechizosAgrupada,
        verbos:  VERBOS_MAGIA.map(v => mapSkill(v.key, v.label)),
        esferas: ESFERAS_MAGIA.map(e => mapSkill(e.key, e.label)),
        baseHechiceria,
        puntosRestantes,
        pmMax:             this._charData.espiritu * 2,
        espiritu:          this._charData.espiritu,
        espirituConsagrado,
        espirituLibre:     this._charData.espiritu - espirituConsagrado
      };
    }
  ...
  _onRender(context, options) {
    ...
    el.querySelectorAll(".cc-hechizo-card").forEach(card => {
      card.addEventListener("click", ev => {
        if (ev.target.closest(".cc-hechizo-perm-row")) return;
    ...
    el.querySelectorAll(".cc-magia-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const delta = parseInt(btn.dataset.delta);
        const libre = this._charData.magiaLibres[skill] ?? 0;
        this._charData.magiaLibres[skill] = Math.max(0, libre + delta);
        this.render();
      });
    });
  ...
  async _crearActor() {
    ...
    if (this._charData.ideologiaReligion) {
      ...
      await actor.update({
        "system.lealtad.ley": pl.ley,
        "system.lealtad.caos": pl.caos,
        "system.lealtad.elementos": pl.elementos,
        "system.lealtad.antepasados": pl.antepasados,
        "system.actitudDivina": this._charData.actitudDivina
      });
    }
    ...
    if (tieneHechiceria || tieneTeurgia) {
      const espConsa = this._charData.hechizosElegidos
        .filter(h => h.permanent).reduce((s, h) => s + (h.pmCoste || 1), 0);
      const magiaUpd = {
        "system.hechiceria.pmMax": this._charData.espiritu * 2,
        "system.hechiceria.pmActual": this._charData.espiritu * 2,
        "system.hechiceria.espirituConsagrado": espConsa
      };
    ...
    if (this._charData.tipoMagia === "don") {
      await Item.create({
        name: "Don de la magia", type: "ventaja",
        system: { coste: -1, tipo: "ventaja", efecto: "El personaje tiene potencial mágico innato pero no ha sido entrenado. Puede aprender hechizos durante el juego.", fuente: "creacion" }
      }, { parent: actor });
    }
