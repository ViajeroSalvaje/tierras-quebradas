...
Movimiento: Correr, rápido.`;
  ...
  static _parsear(raw) {
    ...
    // Secciones
    const secRe = /(Habilidades:|Armas:|Poderes:)/gi;
    ...
    // Habilidades: "Nombre valor, Nombre valor"
    const habilidades = {};
    if (sec.habilidades) {
    ...
    // Poderes: todo el texto de la sección
    const poderes = sec.poderes
      ? sec.poderes.replace(/Movimiento:.*/i, "").trim()
      : "";
    ...
    return { nombre, cuerpo, mente, espiritu, atractivo, tamanyo, fuerza, pvMax, pvGrave, pvLeve, proteccion, proteccionTipo, mDano1m, mDano2m, pm, alImpacto, movimiento, habilidades, armas, poderes, notas };
  }
}
