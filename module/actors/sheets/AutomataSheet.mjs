...
export class AutomataSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  static DEFAULT_OPTIONS = {
    classes: ["tierras-quebradas", "sheet", "actor", "automata"],
    position: {
      width: 600,
      height: 540,
    },
  ...
  static PARTS = {
    form: { template: "systems/tierras-quebradas/templates/actors/automata-sheet.hbs", scrollable: [".sheet-body"] }
  };
  ...
  async _onDropItem(event, data) {
    ...
    if (item.type === "caracteristicaBestiario") {
      if (item.system.tipo !== "debilidad" && item.system.tipo !== "rasgo") { ui.notifications.warn("Solo se pueden añadir Rasgos/Poderes o Debilidades del compendio de Bestiario."); return; }
  ...
  _onRender(context, options) {
    ...
    el.querySelector(".anadir-debilidad")?.addEventListener("click", async () => {
      const item = await Item.create({ name: "Nueva debilidad", type: "caracteristicaBestiario", system: { tipo: "debilidad" } }, { parent: this.actor });
      item?.sheet.render(true);
    });
    ...
    el.querySelector(".pnj-anadir-rasgo")?.addEventListener("click", async () => {
      const item = await Item.create({ name: "Nuevo rasgo / poder", type: "caracteristicaBestiario", system: { tipo: "rasgo" } }, { parent: this.actor });
      item?.sheet.render(true);
    });
