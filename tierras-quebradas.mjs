...
// Agonía: 1 PD por minuto real para actores en agonía (comprueba cada 60s)
Hooks.once("ready", () => {
  setInterval(async () => {
    if (!game.user.isGM) return;
    for (const actor of game.actors) {
      if (!actor?.system?.salud?.agonia) continue;
      const pvActual = actor.system.salud.pvActual?.valor ?? 0;
      const pvMax    = actor.system.salud.pvMax?.valor    ?? 0;
      const nuevoPV  = pvActual - 1;
      await actor.update({ "system.salud.pvActual.valor": Math.max(0, nuevoPV) });
      ChatMessage.create({
        speaker: ChatMessage.getSpeaker({ actor }),
        content: `<p><strong>${actor.name}</strong> pierde 1 PV por <em>Agonía</em> (${Math.max(0, nuevoPV)}/${pvMax} PV).</p>`
      });
      // Si los PD acumulados alcanzan pvMax → muerte
      if (nuevoPV <= -pvMax) {
        await actor.update({ "system.salud.muerto": true, "system.salud.agonia": false });
        ChatMessage.create({
          speaker: ChatMessage.getSpeaker({ actor }),
          content: `<div class="tq-result-card fallo"><div class="tq-resultado-label">☠ Muerte por Agonía</div><p>${actor.name} ha muerto sin ser estabilizado.</p></div>`
        });
      }
    }
  }, 60_000);
